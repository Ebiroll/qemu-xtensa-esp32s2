...
static void esp_uart_update_irq(ESP32S2UARTState *s)
{
    bool irq = false;
...
{
    ESP32S2UARTState *s = ESP32S2_UART(opaque);
    uint64_t r = 0;
...
{
    ESP32S2UARTState *s = ESP32S2_UART(dev);
...
{
    ESP32S2UARTState *s = ESP32S2_UART(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);

    memory_region_init_io(&s->iomem, obj, &uart_ops, s,
                          TYPE_ESP32S2_UART, UART_REG_CNT * sizeof(uint32_t));
...
static Property esp32_uart_properties[] = {
    DEFINE_PROP_CHR("chardev", ESP32S2UARTState, chr),
...
static const TypeInfo esp32_uart_info = {
    .name = TYPE_ESP32S2_UART,
    .parent = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(ESP32S2UARTState),
