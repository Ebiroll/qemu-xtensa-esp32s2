...
{
    Esp32IntMatrixState *s = ESP32S2_INTMATRIX(opaque);
...
static inline uint8_t* get_map_entry(Esp32IntMatrixState* s, hwaddr addr)
{
    ...
    if (source_index > ESP32S2_INT_MATRIX_INPUTS * ESP32S2_CPU_COUNT) {
        error_report("%s: source_index %d out of range", __func__, source_index);
        return NULL;
    }
    int cpu_index = source_index / ESP32S2_INT_MATRIX_INPUTS;
    source_index = source_index % ESP32S2_INT_MATRIX_INPUTS;
...
{
    Esp32IntMatrixState *s = ESP32S2_INTMATRIX(dev);
...
{
    Esp32IntMatrixState *s = ESP32S2_INTMATRIX(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);

    memory_region_init_io(&s->iomem, obj, &esp_intmatrix_ops, s,
                          TYPE_ESP32S2_INTMATRIX, ESP32S2_INT_MATRIX_INPUTS * ESP32S2_CPU_COUNT * sizeof(uint32_t));
    sysbus_init_mmio(sbd, &s->iomem);

    qdev_init_gpio_in(DEVICE(s), esp32_intmatrix_irq_handler, ESP32S2_INT_MATRIX_INPUTS);
}
