...
#define MMU_RANGE_SIZE          (ESP32S2_CACHE_PAGES_PER_REGION * sizeof(uint32_t))
#define MMU_RANGE_LAST          (MMU_RANGE_SIZE - sizeof(uint32_t))

#define PRO_DROM0_MMU_FIRST     (S2_DR_REG_FLASH_MMU_TABLE_PRO - S2_DR_REG_DPORT_BASE)
#define PRO_DROM0_MMU_LAST      (PRO_DROM0_MMU_FIRST + MMU_RANGE_LAST)
#define PRO_IRAM0_MMU_FIRST     (S2_DR_REG_FLASH_MMU_TABLE_PRO - S2_DR_REG_DPORT_BASE + MMU_RANGE_SIZE)
#define PRO_IRAM0_MMU_LAST      (PRO_IRAM0_MMU_FIRST + MMU_RANGE_LAST)
#define APP_DROM0_MMU_FIRST     (S2_DR_REG_FLASH_MMU_TABLE_APP - S2_DR_REG_DPORT_BASE)
#define APP_DROM0_MMU_LAST      (APP_DROM0_MMU_FIRST + MMU_RANGE_LAST)
#define APP_IRAM0_MMU_FIRST     (S2_DR_REG_FLASH_MMU_TABLE_APP - S2_DR_REG_DPORT_BASE + MMU_RANGE_SIZE)
...
2. Area start address: APB_CTRL_X_ACS_n_ADS2_DR_REG, which represents the physical address;
...
static inline void set_mmu_entry(Esp32CacheRegionState* crs, hwaddr base, hwaddr addr, uint64_t val)
{
    ...
    if (val != old_val) {
        crs->mmu_table[(addr - base)/sizeof(uint32_t)] = (val & MMU_ENTRY_MASK) | ESP32S2_CACHE_MMU_ENTRY_CHANGED;
    }
}

static uint64_t esp32s2_dport_read(void *opaque, hwaddr addr, unsigned int size)
{
    //Esp32DportState *s = ESP32S2_DPORT(opaque);
    uint64_t r = 0;
                              ...
                              uint64_t value, unsigned int size)
{
    ...
    case A_DPORT_PRO_CACHE_CTRL:
        if (FIELD_EX32(value, DPORT_PRO_CACHE_CTRL, CACHE_FLUSH_ENA)) {
            ...
            ESP32S2_cache_data_sync(&s->cache_state[0].drom0);
            ESP32S2_cache_data_sync(&s->cache_state[0].iram0);
        }
    ...
    case A_DPORT_APP_CACHE_CTRL:
        if (FIELD_EX32(value, DPORT_APP_CACHE_CTRL, CACHE_FLUSH_ENA)) {
            ...
            ESP32S2_cache_data_sync(&s->cache_state[1].drom0);
            ESP32S2_cache_data_sync(&s->cache_state[1].iram0);
        }
...
static const MemoryRegionOps ESP32S2_dport_ops = {
...
static void esp32s2_cache_data_sync(Esp32CacheRegionState* crs)
{
    ...
    for (int i = 0; i < ESP32S2_CACHE_PAGES_PER_REGION; ++i) {
        ...
        if (!(mmu_entry & ESP32S2_CACHE_MMU_ENTRY_CHANGED)) {
            continue;
        }
        mmu_entry &= MMU_ENTRY_MASK;
        if (mmu_entry & ESP32S2_CACHE_MMU_INVALID_VAL) {
            uint32_t fill_val = crs->type == ESP32S2_DCACHE ? 0xbaadbaad : 0x00000000;
            for (int word = 0; word < ESP32S2_CACHE_PAGE_SIZE / sizeof(uint32_t); ++word) {
                cache_page[word] = fill_val;
            }
        } else {
            uint32_t phys_addr = mmu_entry * ESP32S2_CACHE_PAGE_SIZE;
            blk_pread(crs->cache->dport->flash_blk, phys_addr, cache_page, ESP32S2_CACHE_PAGE_SIZE);
        }
        crs->mmu_table[i] &= ~ESP32S2_CACHE_MMU_ENTRY_CHANGED;
        n++;
    }
    memory_region_flush_rom_device(&crs->mem, 0, ESP32S2_CACHE_REGION_SIZE);
}
...
static void esp32s2_cache_state_update(Esp32CacheState* cs)
{
    ...
    if (!cs->drom0.mem.enabled && drom0_enabled) {
        ESP32S2_cache_data_sync(&cs->drom0);
    }
    ...
    if (!cs->iram0.mem.enabled && iram0_enabled) {
        ESP32S2_cache_data_sync(&cs->iram0);
    }
...
{
    Esp32DportState *s = ESP32S2_DPORT(dev);
...
static void esp32s2_dport_realize(DeviceState *dev, Error **errp)
{
    ...
    //for (int index = 0; index < ESP32S2_DPORT_CROSSCORE_INT_COUNT; ++index) {
...
{
    Esp32DportState *s = ESP32S2_DPORT(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);

    memory_region_init_io(&s->iomem, obj, &ESP32S2_dport_ops, s,
                          TYPE_ESP32S2_DPORT, 0x1000 /* S2_DR_REG_DMA_COPY_BASE-S2_DR_REG_SENSITIVE_BASE*/);
...
*/
    ...
    qdev_init_gpio_out_named(DEVICE(sbd), &s->clk_update_req, ESP32S2_DPORT_CLK_UPDATE_GPIO, 1);
}
...
static void esp32s2_dport_class_init(ObjectClass *klass, void *data)
{
    ...
    device_class_set_props(dc, esp32s2_dport_properties);
}

static const TypeInfo esp32s2_dport_info = {
    .name = TYPE_ESP32S2_DPORT,
