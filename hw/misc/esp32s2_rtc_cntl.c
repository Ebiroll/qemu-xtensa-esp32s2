...
static void esp32s2_rtc_update_cpu_stall(Esp32S2RtcCntlState* s);
static void esp32s2_rtc_update_clk(Esp32S2RtcCntlState* s);

static uint64_t esp32s2_rtc_cntl_read(void *opaque, hwaddr addr, unsigned int size)
{
    Esp32S2RtcCntlState *s = ESP32S2_RTC_CNTL(opaque);
    uint64_t r = 0;
    switch (addr) {
...
static void esp32s2_rtc_cntl_write(void *opaque, hwaddr addr, uint64_t value,
                                 unsigned int size)
{
    Esp32S2RtcCntlState *s = ESP32S2_RTC_CNTL(opaque);
    switch (addr) {
    case A_RTC_CNTL_OPTIONS0:
        if (value & R_RTC_CNTL_OPTIONS0_SW_SYS_RESET_MASK) {
            s->reset_cause[0] = ESP32S2_SW_SYS_RESET;
            s->reset_cause[1] = ESP32S2_SW_SYS_RESET;
        ...
        if (value & R_RTC_CNTL_OPTIONS0_SW_APPCPU_RESET_MASK) {
            s->reset_cause[1] = ESP32S2_SW_CPU_RESET;
...
static const MemoryRegionOps esp32s2_rtc_cntl_ops = {
    .read =  esp32s2_rtc_cntl_read,
    .write = esp32s2_rtc_cntl_write,
    .endianness = DEVICE_LITTLE_ENDIAN,
};

static void esp32s2_rtc_cntl_reset(DeviceState *dev)
{
    Esp32S2RtcCntlState *s = ESP32S2_RTC_CNTL(dev);


    s->time_base_ns = qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL);
}

static void esp32s2_rtc_cntl_realize(DeviceState *dev, Error **errp)
{
}

static void esp32s2_rtc_cntl_init(Object *obj)
{
    Esp32S2RtcCntlState *s = ESP32S2_RTC_CNTL(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);

    memory_region_init_io(&s->iomem, obj, &esp32s2_rtc_cntl_ops, s,
                          TYPE_ESP32S2_RTC_CNTL, ESP32S2_RTC_CNTL_SIZE);
    sysbus_init_mmio(sbd, &s->iomem);
    sysbus_init_irq(sbd, &s->irq);
    qdev_init_gpio_out_named(DEVICE(sbd), &s->dig_reset_req, ESP32S2_RTC_DIG_RESET_GPIO, 1);
    qdev_init_gpio_out_named(DEVICE(sbd), &s->cpu_reset_req[0], ESP32S2_RTC_CPU_RESET_GPIO, ESP32S2_CPU_COUNT);
    qdev_init_gpio_out_named(DEVICE(sbd), &s->cpu_stall_req[0], ESP32S2_RTC_CPU_STALL_GPIO, ESP32S2_CPU_COUNT);
    qdev_init_gpio_out_named(DEVICE(sbd), &s->clk_update, ESP32S2_RTC_CLK_UPDATE_GPIO, 1);

    for (int i = 0; i < ESP32S2_CPU_COUNT; ++i) {
        s->reset_cause[i] = ESP32S2_POWERON_RESET;
        s->stat_vector_sel[i] = true;
    }

    s->rtc_slowclk = ESP32S2_SLOW_CLK_RC;
    s->rtc_fastclk = ESP32S2_FAST_CLK_8M;
    s->soc_clk = ESP32S2_SOC_CLK_XTAL;
    s->xtal_apb_freq = 40000000;
    s->pll_apb_freq = 80000000;
    esp32s2_rtc_update_clk(s);
}

static Property esp32s2_rtc_cntl_properties[] = {
    DEFINE_PROP_END_OF_LIST(),
};

static void esp32s2_rtc_cntl_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->reset = esp32s2_rtc_cntl_reset;
    dc->realize = esp32s2_rtc_cntl_realize;
    device_class_set_props(dc, esp32s2_rtc_cntl_properties);
}

static const TypeInfo esp32s2_rtc_cntl_info = {
    .name = TYPE_ESP32S2_RTC_CNTL,
    .parent = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(Esp32S2RtcCntlState),
    .instance_init = esp32s2_rtc_cntl_init,
    .class_init = esp32s2_rtc_cntl_class_init
};

static void esp32s2_rtc_cntl_register_types(void)
{
    type_register_static(&esp32s2_rtc_cntl_info);
}

type_init(esp32s2_rtc_cntl_register_types)
